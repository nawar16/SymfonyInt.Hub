...
onMounted(async () => {
  try {
    const res = await axios.get<Log[]>('http://localhost:8000/api/logs')
    logs.value = Array.isArray(res.data) ? res.data : (res.data as any)['hydra:member'] || []
  } catch (err) {
...
const formatDate = (dateString: string) => {
  return new Date(dateString).toLocaleString('de-DE') // 24.12.2023, 14:30:00
}
</script>
